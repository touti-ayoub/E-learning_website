  ...
  template: `
    <div class="course-access-container">
      ...
      <div *ngIf="checkComplete && !hasAccess" class="access-denied">
        ...
        <p>{{ accessMessage }}</p>
        <button (click)="requestAccess()" [disabled]="requestingAccess" class="request-button">
...
export class CourseAccessComponent implements OnInit {
  ...
  checkAccess(): void {
    ...
    this.courseAccessService.checkCourseAccess(this.userId, this.courseId)
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          ...
          this.accessMessage = 'Unable to verify course access. Please try again later.';
          this.checkComplete = true;
        }
      });
  }
  ...
  requestAccess(): void {
    ...
    this.courseAccessService.requestCourseAccess(this.userId, this.courseId)
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          ...
          this.requestComplete = true;
          this.requestingAccess = false;
          this.requestMessage = 'Failed to request access. Please try again later.';
        }
      });
  }
} 