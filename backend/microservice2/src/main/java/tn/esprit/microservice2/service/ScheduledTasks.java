...
public class ScheduledTasks {
    ...
    public void checkInstallmentDueDates() {
        ...
        // Handle different levels of late payments
        handleOneDayLatePayments();
        handleFiveDaysLatePayments();
        handleFifteenDaysLatePayments();
    }*/

     /**
     * Handle payments that are exactly 1 day late
     * Action: Send email notification
     */
    /*@Transactional
    public void handleOneDayLatePayments() {
        LocalDate oneDayLate = LocalDate.now().minusDays(1);

        List<PaymentSchedule> oneDayLateSchedules = paymentScheduleRepository
                .findByStatusAndDueDateWithDetails(PaymentScheduleStatus.OVERDUE, oneDayLate);


        logger.info("Found {} payments that are exactly 1 day late", oneDayLateSchedules.size());

        for (PaymentSchedule schedule : oneDayLateSchedules) {
            try {
                User user = schedule.getPayment().getSubscription().getUser();
                String courseTitle = schedule.getPayment().getSubscription().getCourse().getTitle();
                Double amount = schedule.getAmount();
                ...
                logger.error("Error sending 1-day late notice for schedule {}: {}",
                        schedule.getId(), e.getMessage(), e);
            }
        }
    }*/
    ...
    /**
     * Handle payments that are exactly 5 days late
     * Action: Apply 5% penalty and update total amount
     */
   /* @Transactional

    public void handleFiveDaysLatePayments() {
        LocalDate fiveDaysLate = LocalDate.now().minusDays(5);

        List<PaymentSchedule> fiveDaysLateSchedules = paymentScheduleRepository
                .findByStatusAndDueDate(PaymentScheduleStatus.OVERDUE, fiveDaysLate);

        logger.info("Found {} payments that are exactly 5 days late", fiveDaysLateSchedules.size());

        for (PaymentSchedule schedule : fiveDaysLateSchedules) {
            try {
                // Apply 5% penalty if not already applied
                if (schedule.getPenaltyAmount() == 0) {
                    // Store original amount before changing it
                ...
                logger.error("Error applying penalty to schedule {}: {}",
                        schedule.getId(), e.getMessage(), e);
            }
        }
    }*/
    ...
    /**
     * Handle payments that are exactly 15 days late
     * Action: Suspend access to course (change subscription status)
     */
   /* @Transactional

    public void handleFifteenDaysLatePayments() {
        LocalDate fifteenDaysLate = LocalDate.now().minusDays(15);

        List<PaymentSchedule> fifteenDaysLateSchedules = paymentScheduleRepository
                .findByStatusAndDueDate(PaymentScheduleStatus.OVERDUE, fifteenDaysLate);

        logger.info("Found {} payments that are exactly 15 days late", fifteenDaysLateSchedules.size());

        for (PaymentSchedule schedule : fifteenDaysLateSchedules) {
            try {
                Subscription subscription = schedule.getPayment().getSubscription();
                ...
                logger.error("Error suspending subscription for schedule {}: {}",
                        schedule.getId(), e.getMessage(), e);
            }
        }
    }
